<?php 
	...
	$query1 = "select disease_name,patient_email,progress1,progress2,progress3 from treatment where patient_email='".$doctor_name."'";
	if($query_run1 = $mysqli->query($query1))
	{	
		while($row = mysqli_fetch_array($query_run1))
		{
			array_push($disease_name, $row["disease_name"]);
			array_push($patient_name, $row["doctor_email"]);
